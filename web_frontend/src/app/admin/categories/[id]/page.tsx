...
export default function EditCategoryPage() {
  ...
  useEffect(() => {
    async function fetchCategory() {
      setLoading(true);
      const result = await getCategoryById(params.id as string);
      ...
      if (result.success && result.data) {
        ...
        if (c.modifiers && Array.isArray(c.modifiers)) {
          setModifiers(c.modifiers);
        }
      } else {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-gray-800 mb-8">Edit Category</h1>
